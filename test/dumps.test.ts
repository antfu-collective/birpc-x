import type { RpcDumpRecord } from '../src/types'
import * as v from 'valibot'
import { describe, expect, it } from 'vitest'
import { createClientFromDump, createDefineWrapperWithContext, defineRpcFunction, dumpFunctions } from '../src'
...
describe('dumps', () => {
  ...
  it('should throw error for non-matching args when fallback is not provided', async () => {
    ...
    await expect(client.greet('Alice')).resolves.toBe('Hello, Alice!')
    await expect(client.greet('Unknown')).rejects.toThrow('[birpc-x] No dump match for "greet"')
  })
  ...
  it('should throw error for non-existent function', async () => {
    ...
    expect(() => (client as any).subtract(1, 2)).toThrow('[birpc-x] Function "subtract" not found in dump store')
  })
  ...
  it('should throw error if action type function has dump', async () => {
    ...
    await expect(dumpFunctions([sendEmail])).rejects.toThrow(
      '[birpc-x] Function "sendEmail" with type "action" cannot have dump configuration',
    )
  })
  ...
  it('should throw error if event type function has dump', async () => {
    ...
    await expect(dumpFunctions([notifyUser])).rejects.toThrow(
      '[birpc-x] Function "notifyUser" with type "event" cannot have dump configuration',
    )
  })
